...
### 3.4 Document Processing Pipeline
    ...
    Ingest – watch B2 or scrape commission sites, enqueue new files.

    Extract – parse text (pdfplumber/unstructured); OCR if needed.

    Chunk & normalise – ~1 k‑token segments with page refs.

    Index – write to document_content, build tsvector + embeddings.

    Enrich – run LLM summariser and entity extractor.

    Log & monitor – metrics into Prometheus; job status into Supabase.
...
quality_check	Verified? (*“pending”
...
## 5 Getting Started (3‑Step Quick Start)
    ...
    cd cairn && pip install -r requirements.txt

    Provision services – Supabase project, Backblaze B2 bucket, Google Cloud service account.

    Add secrets – copy .env.example → .env and fill keys.

Run python scripts/sheet_to_supabase.py to bootstrap an empty Sheet and verify credentials.
