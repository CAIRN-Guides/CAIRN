...
# Full drop-in replacement code - v1.11 (Batch Download)
# Replaces single-file download with batch download for displayed files.
# REQUIRES a new backend endpoint (/documents/batch-download-url) to handle ZIP generation.
# Removes getRowId config. Uses selected_rows for details pane only.
# Uses simplified update/return modes, reload_data=False removed.
# Fixes ValueError using .empty. Ensures PK_ID is string.
...
import traceback
import json # For sending list of IDs to backend
...
DOWNLOAD_API_TIMEOUT_SECONDS = 120 # Increased timeout for potential ZIP generation
...
with search_basic_tab:
    st.info("Basic search view. Use sidebar filters and click 'Load Documents'. Use buttons below the table to download.")
            ...
            with col_batch_btn:
                if displayed_df_basic is not None and not displayed_df_basic.empty:
                    ...
                    if 'PK_ID' in displayed_df_basic.columns:
                        batch_pk_ids_basic = displayed_df_basic['PK_ID'].tolist()
                        handle_batch_download(batch_pk_ids_basic, "basic")
                    else:
                        st.warning("PK_ID column not found in displayed data for batch download.")
                else:
                    st.button("⬇️ Download Displayed Files as ZIP", disabled=True, key="download_batch_basic_disabled")
...
with search_advanced_tab:
    st.info("Advanced search view with all columns. Use sidebar filters and click 'Load Documents'. Use buttons below the table to download.")
            ...
            with col_batch_btn_adv:
                if displayed_df_advanced is not None and not displayed_df_advanced.empty:
                    if 'PK_ID' in displayed_df_advanced.columns:
                        batch_pk_ids_advanced = displayed_df_advanced['PK_ID'].tolist()
                        handle_batch_download(batch_pk_ids_advanced, "advanced")
                    else:
                        st.warning("PK_ID column not found in displayed data for batch download.")
                else:
                    st.button("⬇️ Download Displayed Files as ZIP", disabled=True, key="download_batch_advanced_disabled")
